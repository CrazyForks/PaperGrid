...
export default async function AboutPage() {
  ...
  const canShowEmail =
    Boolean(contactEmail) && Boolean(showSocialEmail ?? true) && isValidHref(contactEmail)
  const canShowGithub =
    Boolean(contactGithub) && Boolean(showSocialGithub ?? true) && isValidHref(contactGithub)
  const canShowX = Boolean(contactX) && Boolean(showSocialX ?? true) && isValidHref(contactX)
  const canShowBilibili =
    Boolean(contactBilibili) && Boolean(showSocialBilibili ?? true) && isValidHref(contactBilibili)
        ...
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-8 lg:grid-cols-3">
              ...
              <Card>
                <CardContent className="p-6">
                  <div className="flex flex-col items-center text-center">
                    {/* 头像 */}
                    <AboutAvatarTrigger>
                      <Avatar className="mb-4 h-32 w-32 cursor-pointer border-2 border-gray-900 dark:border-white">
                        <AvatarImage src={defaultAvatarUrl || undefined} />
                        <AvatarFallback className="bg-gray-50 font-serif text-5xl font-bold text-gray-900 dark:bg-gray-800 dark:text-white">
                    ...
                    {/* 职位 */}
                    <p className="mb-4 text-gray-600 dark:text-gray-400">{role || '全栈开发者'}</p>
            ...
            <div className="space-y-6 lg:col-span-2">
              ...
              <Card>
                <CardContent className="p-6">
                  <h3 className="mb-4 text-xl font-bold text-gray-900 dark:text-white">个人简介</h3>
                  <div className="prose prose-gray dark:prose-invert max-w-none">
                    {effectiveBioParagraphs.map((p, idx) => (
                      <p
                        key={idx}
                        className={`leading-relaxed text-gray-700 dark:text-gray-300 ${idx > 0 ? 'mt-4' : ''}`}
                      >
                        {p}
                      </p>
                    ))}
              ...
              <Card>
                <CardContent className="p-6">
                  <h3 className="mb-4 text-xl font-bold text-gray-900 dark:text-white">技术栈</h3>
              ...
              <Card>
                <CardContent className="p-6">
                  <h3 className="mb-4 text-xl font-bold text-gray-900 dark:text-white">兴趣爱好</h3>
                  <div className="prose prose-gray dark:prose-invert max-w-none">
              ...
              <Card>
                <CardContent className="p-6">
                  <h3 className="mb-4 text-xl font-bold text-gray-900 dark:text-white">联系我</h3>
                  <div className="prose prose-gray dark:prose-invert max-w-none">
                    <p className="leading-relaxed text-gray-700 dark:text-gray-300">
                      {contactIntro ||
                        '如果你想与我交流技术问题、合作项目,或者只是打个招呼,欢迎通过以下方式联系我:'}
                    </p>
                    <div className="mt-4 space-y-2">
                      {canShowEmail && (
                        <p className="text-gray-700 dark:text-gray-300">
                          📧 Email:{' '}
                          <a href={`mailto:${contactEmail}`} className="pg-about-contact-link">
                            {contactEmail}
                          </a>
                        </p>
                      )}
                      {canShowGithub && (
                        <p className="text-gray-700 dark:text-gray-300">
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="pg-about-contact-link"
                          >
                            {contactGithub}
                          </a>
                        </p>
                    ...
                    <p className="mt-4 text-gray-700 dark:text-gray-300">感谢你的访问!</p>
                  </div>
                </CardContent>
              </Card>
            </div>
